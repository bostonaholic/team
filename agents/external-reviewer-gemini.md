...
1. **Availability probe.** Run:
   ...
   `docs/plans/<id>/reviews/external-reviewer-gemini.md` using the
...
3. **CLI invocation.** Build the prompt as a file (so neither the diff
   ...
   stdout="$(timeout 300 gemini < "$prompt_file" 2>/dev/null)"
   exit_code=$?
   ```
   ...
   equivalent `timeout 300 gemini --file "$prompt_file" 2>/dev/null`
...
- `gemini timed out after 300s`
- `gemini returned non-zero with no output`
- `gemini returned auth error` (only if auth failure is detected
