...
- **Multi-model adversarial review in the IMPLEMENT verification fan-out.** The five Claude reviewers (`code-reviewer`, `security-reviewer`, `verifier`, `ux-reviewer`, `technical-writer`) are now joined by two opt-in external CLI wrappers — `external-reviewer-codex` and `external-reviewer-gemini` — and a `review-aggregator` synthesis agent. Each external wrapper probes its CLI with `command -v` and fails open: if the CLI is absent (or auths/timeouts), the wrapper writes a SKIP artifact and exits 0, so the pipeline is byte-identical to today for users who haven't installed either tool. When a CLI is present, the wrapper feeds the diff to it via stdin (no shell-word interpolation), captures stdout, and normalizes findings into the existing Conventional Comments format. The aggregator reads `docs/plans/<id>/reviews/` plus the five Claude transcripts, fuzzy-matches findings on `(file:line, kind, summary keywords)`, and emits a single ranked synthesis tagging each finding `corroborated by N/M models` — single-model findings carry an `[single-model — extra scrutiny]` flag and the most-severe kind survives on collision. **Hard-gate semantics are preserved verbatim**: external reviewers and the aggregator are ADVISORY, never in `hardGates`; the existing trio (`security-reviewer` CRITICAL/HIGH, `verifier` any failure, `code-reviewer` REQUEST CHANGES) still gates, and confidence tags are display-only. A new `skills/review-aggregation/SKILL.md` carries the methodology; the orchestrator clears `docs/plans/<id>/reviews/` before every fan-out (with an `ID_RE` guard against path traversal) and a `.claude/scripts/check-aggregator-fixture.sh` suite pins seven contract fixtures (corroboration math, SKIP/PARTIAL denominator rules, kind-promotion, never-aggregated-away hard-gate, etc.). Inspired by `yeachan-heo/oh-my-claudecode`'s artifact-first synthesis pattern.
