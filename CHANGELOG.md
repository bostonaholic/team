...
- **IMPLEMENT phase now enforces strict red-green-refactor with role-separated agents.** The slice loop runs `test-architect` (per slice, writes the slice's failing acceptance tests — red) → mechanical red gate → `greener` (per slice, smallest necessary code to turn tests green) → mechanical green gate (3-attempt cap, "prior slices still pass" semantics) → `refactorer` (per slice, optional `refactor:` commit; self-verifies green after each change and reports no-op if it cannot leave green) → next slice → 5 reviewers. Each slice produces up to three commits in fixed order: `test:`, `feat:`, optional `refactor:`. The `implementer` agent is retained strictly for the typed-failure-class review-fix loop after the 5-reviewer aggregate gate. Agent inventory grows from 13 to 15. Documented end-to-end across `skills/team/SKILL.md`, `skills/team-implement/SKILL.md`, `skills/qrspi-workflow/SKILL.md`, `agents/{test-architect,greener,refactorer,implementer}.md`, `README.md`, `AGENTS.md`, `docs/architecture.md`, and `docs/index.md`. Locked in by `tests/acceptance/red-green-refactor-tdd.sh`.
