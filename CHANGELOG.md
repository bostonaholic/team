...
- The code review briefs are shorter. The code, security, documentation, and UX reviewers read about 15% less review-brief text each round, and `/code-review` reads about 12% less of its own files. The gates, severity tiers, report format, screenshot rules, and commands are unchanged. **What this asks of you:** nothing.
...
- **`pr-rebase` now reconciles a conflicted generated file to the smallest correct change.** A lockfile, `structure.sql`, or compiled asset that conflicts during the rebase is restored from the rebase's current copy and given only the direct change, or regenerated with the project's own frozen/consistent command, and the result is checked for a minimal diff. A non-frozen install that re-resolves unrelated entries is no longer the path. **What this asks of you:** nothing.
...
- **Team takes up less room in the host's shared skills catalog.** Hosts render every installed skill — Team's, every other plugin's, and their own — into one list on a fixed budget. Codex caps that list at 8,000 characters, or 2% of the context window in tokens, and over the cap it shortens every skill's description, Team's and its neighbors' alike, then starts dropping skills off the list. Team ships 90 skills and cannot see who else is installed, so it cannot know whether it is the reason a catalog overflowed; what it can do is stop spending the pool on words that earn nothing. Thirteen descriptions listed three trigger phrases where two of them meant the same thing. The duplicates are gone: 11,344 description characters down to 10,975. Every skill keeps at least two distinct triggers plus its slash command, and no first sentence changed. **What this asks of you:** nothing. If a host warned that skill descriptions were shortened, Team is now a smaller part of that total, though the warning depends on everything else you have installed.
...
- **A new standalone utility, [`/pr-screenshots`](https://github.com/bostonaholic/team/blob/main/skills/pr-screenshots/SKILL.md), attaches local images to any pull request's body.** Until now the only way this plugin could put an image into a PR description was to run the whole QRSPI pipeline: the procedure lived inside `team-pr`, it fired only when `ux-reviewer` had written a capture manifest, and it took no PR number or URL, so a session that simply wanted screenshots on PR 1234 had no way to ask. The new skill takes a PR number or URL and a list of image paths with captions, uploads each file through GitHub's own attachment pipeline, and writes one `## Screenshots` section into the body — then reads the body back through GitHub's renderer and reports whether the images actually render. Open, draft, closed, and merged PRs are all in scope, private repositories included, and a run needs no browser, no second sign-in, and no pipeline artifacts. Every failure path still ends with an open PR and a visible note, and every entry is accounted for: one that landed a URL is listed with it, one that did not is listed under `Not uploaded:` with the failure class that stopped it, so a run where nothing uploads can never report an empty failure list beside a body claiming the images were only captured. The body edit is deliberately conservative: it models a closed set of markdown constructs and refuses any body carrying something outside that set — an unterminated HTML comment, a heading indented into a code block, a raw HTML tag anywhere on a line (a `<div>`, a `<table>`, a `<details>`, or a `<video>` in a list item or mid-sentence, not only at column zero), an image written as `<img>`, `![alt][ref]`, or a bare auto-embedded URL, and anything inside the Screenshots section that the skill did not write, prose included — leaving the body byte-identical and naming the reason, the line number, and the offending text, because a duplicate is recoverable and a deletion from a merged PR is not. "The skill did not write it" is decided by provenance rather than by shape: notes are rendered under a `> _note:_` marker a reviewer would not type, and a `**bold**` line counts as one of the skill's captions only where the renderer emits one, so a reviewer's own blockquote or bold warning under the heading refuses exactly as a bare sentence does. An escaped backslash buys no raw HTML past that refusal either — `\\<img …>` is an escaped backslash followed by a live tag, and GitHub renders it as a real image, so the escape now holds only for an odd run of backslashes. A plain markdown link in the section is refused for the same reason — `[click me](https://evil.example)` is neither raw HTML nor an image, so nothing in the code stood behind the escaping until now. Every one of those refusals is computed from the body *before* the first upload, so a body the skill cannot edit costs no attachment at all, and a refusal report names the manual edit that clears it. Uploaded URLs are accepted only from the host the PR itself resolved to, from the single enumerated attachment-proxy host (`private-user-images.githubusercontent.com`, or `private-user-images.<enterprise-host>`), or from a GitHub Enterprise asset host named in `PR_SCREENSHOTS_ASSET_HOST`, and only at that host's own `/user-attachments/assets/` path, or the one- or two-segment image path the private-repository proxy rewrite actually uses — a URL that merely carries `user-attachments` somewhere in it is refused, so is one whose path walks back out through a `..` segment or a `%2e`/`%2f` encoding of one, and so is one on any other `githubusercontent.com` subdomain, since `raw.githubusercontent.com` serves any public repository's content. If any re-read of the PR body fails part way through an upload, the run stops before writing rather than composing a new description from a baseline it can no longer prove current: the images are live, the description is untouched, and the report says exactly what is on the PR and what to edit by hand. **What this asks of you:** nothing, unless you want it — run `/pr-screenshots <pr-url>` and name the images. A `gh` older than 2.100.0 has no attach support, in which case the run says so in its report and writes the captions and the captured file names as text instead.

- **The pointers into the screenshot machinery now resolve, and the orphan-branch trap is written down where someone about to fall into it will read it.** [`/pr-open-comments`](https://github.com/bostonaholic/team/blob/main/skills/pr-open-comments/SKILL.md) offers `/pr-screenshots` as an option when a reviewer asks for a screenshot — in the operator's menu only, never on the auto-apply path, because the image has to be captured first and a PR comment is untrusted data. `docs/architecture.md` points at the skill that does the work instead of a browser sign-in that no longer exists, and the stale pointer at a README section that was never written is gone. `pr-screenshots` records why committing images to an orphan branch and linking the blob URL is not an alternative: the images die with the branch, and on a private repo a token-authenticated fetch of that web-session URL returns 404 whether the blob is there or not, so it can never be evidence the image renders. **What this asks of you:** nothing.

- **In a multi-repo run, every companion PR's screenshot section is now checked against that companion's own rendered body.** The section still comes from one upload on the home repository's PR — never one upload per repo — and is spliced into each companion through the same committed pure function the home write uses, so the footer and the closing line survive the edit byte-identical. Each companion write is then read back through GitHub's renderer at that companion's own host, owner, repo, and number. Every call in that loop carries the guards the home write carries: the pre-image read is checked for a JSON envelope rather than trusted to be non-empty, the spliced body file is bound per companion so a refused companion can never be handed the previous one's description, the write is gated on the pre-image still being current, and the host the companion lives on is named on all three calls — without it, an Enterprise companion's edit and read-back both resolve against github.com. A companion that does not render is named in the report and left as written, never reverted and never retried. **What this asks of you:** nothing.

- **[`/team-pr`](https://github.com/bostonaholic/team/blob/main/skills/team-pr/SKILL.md) now delegates screenshot upload instead of carrying its own copy of it.** The upload procedure, the attachment-host knowledge, and the `## Screenshots` wording lived in two places at once — `team-pr`'s upload reference and its body template — so a change to either could drift from the other. `team-pr` now keeps only the policy it owns: whether to run, when, which manifest entries qualify, and what to do with the result. Everything below that is one call to `pr-screenshots`. **What this asks of you:** nothing — a pipeline run behaves exactly as before.
...
- **Six entry points now trigger only on stated intent: `/team`, `/team-implement`, `/team-pr`, `/team-worktree`, `/pr-open-comments`, and `/pr-watch-as-author`.** Each one commits, pushes, opens a PR, creates a branch, or moves a ticket, and each description now says so and names the phrases (or the pipeline advance) that invoke it, on the model `/team-fix` and `/shipit` already used. **What this asks of you:** say what you mean — "run the pipeline", "open the PR", "watch the PR" — rather than expecting a plain "fix this" or "the PR has comments" to start one of them.
- **Every rule Team states now reads without an exception clause.** The audit that followed 0.75.0 found sixty-nine "with one exception", "is exempt", "carve-out", and "stated deviation" clauses across the docs, skills, agents, and dev tooling. Each rule is restated so it holds as written: `security-reviewer` runs on `opus` because security review is complex work, with the never-override-to-Fable constraint moved to the override recipe; an agent may preload more than three methodology skills when a recorded reason says why; the guard class for side-effecting entry points is a predicate any author can re-derive; and the `--` terminator, working-tree, re-arm, and tracker-argv rules each state their own scope. Five build-time tripwires keep the clauses from coming back, including a sweep that fails the build on exception vocabulary in rule prose. **What this asks of you:** nothing — no runtime behavior changed.
- **A phase no longer pays for expensive reasoning it does not use, and the helpers an agent spawns no longer inherit its price.** An `Agent` call that passes no `model:` inherits the caller's model *and* effort. Measured over six weeks of local sessions: 1,140 of 1,297 spawns — 88% — named no model, so an opus agent that spawned ten helpers to locate files paid the opus rate ten times for grep work. Two changes, no phase agent's model tier among them. First, four effort levels drop: [`design-author`](https://github.com/bostonaholic/team/blob/main/agents/design-author.md) and [`structure-planner`](https://github.com/bostonaholic/team/blob/main/agents/structure-planner.md) from xhigh to high, [`researcher`](https://github.com/bostonaholic/team/blob/main/agents/researcher.md) from high to medium, and [`technical-writer`](https://github.com/bostonaholic/team/blob/main/agents/technical-writer.md) from medium to low. Every agent keeps the model it had. Second, the read-only helpers an agent spawns mid-task now carry an explicit tier: [`nested-agents`](https://github.com/bostonaholic/team/blob/main/skills/nested-agents/SKILL.md) states haiku for file location and grep sweeps, sonnet for tracing an unfamiliar subsystem and for skeptics checking one claim, and opus only after a cheaper helper returned inconclusive on the same errand — with xhigh ruled out for a helper answering one bounded question. The `Explore` fan-outs in [`how`](https://github.com/bostonaholic/team/blob/main/skills/how/SKILL.md) and [`why`](https://github.com/bostonaholic/team/blob/main/skills/why/SKILL.md) name `model: sonnet` rather than inheriting. Running against that, the two adversarial design-review dispatches now pin `model: opus` explicitly instead of inheriting it, so no cheaper default — a machine-wide setting, or an orchestrator following the new helper rule — can quietly weaken that gate. **What this asks of you:** nothing. Phase order, artifacts, model tiers, and every review gate are unchanged. A helper returns a shorter map than it used to; the agent that reads it still reasons at the same tier.
...
- **Three methodology skills were renamed into the `principle-` namespace:** `solid-principles` is now [`principle-solid`](https://github.com/bostonaholic/team/blob/main/skills/principle-solid/SKILL.md), `product-thinking` is now [`principle-product-thinking`](https://github.com/bostonaholic/team/blob/main/skills/principle-product-thinking/SKILL.md), and `systems-thinking` is now [`principle-systems-thinking`](https://github.com/bostonaholic/team/blob/main/skills/principle-systems-thinking/SKILL.md). Their content is unchanged, and every agent and skill that loads them was updated in the same change. **What this asks of you:** nothing, unless your own prompts referenced one by its old name — the old names no longer resolve.
...
- **`/reflect` mines the session you are in for the learnings worth keeping.** A long session teaches things that die with it: the guidance that was missing, the command that cost four retries, the thing you did that no skill describes. Run [`/reflect`](https://github.com/bostonaholic/team/blob/main/skills/reflect/SKILL.md) at the end of one and it reads that session's own transcript — not its own memory, which compaction has already thinned — and reports an Accepted / Rejected / Backlog list with the evidence behind every item. Resolution is by **marker**, not by guessing which transcript is yours: the run prints its `mktemp -d` cache path, the host records that line inline in the transcript, and a bundled script finds the one file on disk holding that string. Two matches stop the run rather than hand three reading passes a stranger's session, and the search returns file names only, so no unmatched session's content is read. Three lenses go over the normalized transcript in parallel — **judgment** (guidance that was absent, ambiguous, or misleading), **tooling** (a command or test that cost retries the task did not warrant), **divergent** (something no skill describes) — and the sort into buckets happens once afterwards, so one finding cannot come back classified three ways. Transcript spans are treated as untrusted content throughout: every proposal paraphrases and cites a file path or a turn index, and none quotes a transcript line into a file or an issue body. Say yes and the accepted items become real skill edits, in a later turn that reads the plan file rather than its own memory of the conversation. That turn is fenced in code, not in advice: a proposed skill name must match the shape every skill on disk already has, an edit lands in the skills root the running host actually loads, a new skill only ever appears at `<repo>/.claude/skills/<name>/SKILL.md`, and every resolved real path has to stay inside the repository so a symlinked directory cannot carry a write out of it. The two kinds of write carry the preconditions their undos need: an edit is applied only while its target is tracked, clean, and still byte-identical to the pre-image the plan recorded — which is the whole reason `git restore -- <path>` is a true undo here and not a way to lose your own uncommitted work — and a new skill is written only where nothing exists yet, so deleting the file reflect created undoes it. Anything else is skipped with a reason. After the writes it runs your repo's own check and reports the verdict without fixing or reverting anything. A finding a machine check would enforce better than prose does not become prose: it goes to whatever tracker your repo already names — its own router first, so a repo pointing at Linear or Jira is not quietly fixed to GitHub issues, then an authenticated `gh`, then printing the items verbatim and telling you they went unfiled. Creating an issue is public and irreversible, so each one takes its own question, and a filed issue carries the board fields your router asks for rather than landing untriaged. **What this asks of you:** invoke it deliberately — `reflect` sets `disable-model-invocation: true`, so no model starts a run on its own, and the reporting pass writes nothing outside the run cache whose absolute path it prints. On Codex, which ignores that key, delete `skills/reflect` after installing (the install now says so).
...
- **A resolved thread is no longer taken at its word, and a plain comment needs a push behind it.** 0.52.0 made `isResolved` the wait gate and added a re-review on substance; it still let the flag stand as the finding of fact for what the branch contained. Both shapes are now verified against the current state of the branch, and the two differ only in which way an unclear read falls. A plain comment carries a hard precondition: the head SHA must have advanced after the comment's `createdAt`. A comment raises something about the code, so nothing but the code changing can settle it — a reply alone never does, not a "good catch", not a "fixed in the next push", not an argument that holds. Absent a later push the verdict is **pending**, a fourth verdict that blocks the approval without stopping the watch, which is also the path a freshly posted comment takes at cycle 0 so a new comment never trips the rejected stop on its first poll. Where a plain comment defaults to pending on an unclear read, a resolved thread defaults to accepted: overturning the author's explicit assertion takes *very high confidence* that the concern is unaddressed **and** *strong disagreement* with the resolution. A partial fix worth quibbling with, a different approach than you would have taken, or a fix you cannot confirm either way is an accept — reasoning "this is probably fine, but" is an accept. **What this asks of you:** a watch armed over a plain comment that the author answers only in prose, with no code change, will run to the 24-hour timeout, because a reply cannot satisfy the head-advance precondition. The arm report now says so when the tracked set contains a plain comment, so you can read the reply and approve by hand instead of waiting it out.
