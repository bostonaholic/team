...
- **IMPLEMENT phase now enforces strict red-green-refactor with role-separated agents.** The slice loop runs `red-author` (per slice, writes the slice's failing acceptance tests — red) → mechanical red gate → `green-author` (per slice, smallest necessary code to turn tests green) → mechanical green gate (3-attempt cap, "prior slices still pass" semantics) → `refactor-author` (per slice, optional `refactor:` commit; self-verifies green after each change and reports no-op if it cannot leave green) → next slice → 5 reviewers. Each slice produces up to three commits in fixed order: `test:`, `feat:`, optional `refactor:`. The `implementer` agent is retained strictly for the typed-failure-class review-fix loop after the 5-reviewer aggregate gate. Agent inventory grows from 13 to 15. Documented end-to-end across `skills/team/SKILL.md`, `skills/team-implement/SKILL.md`, `skills/qrspi-workflow/SKILL.md`, `agents/{red-author,green-author,refactor-author,implementer}.md`, `README.md`, `AGENTS.md`, `docs/architecture.md`, and `docs/index.md`. Locked in by `tests/acceptance/red-green-refactor-tdd.sh`.

- **Edge-case enumeration is mandatory in design and tests.** The `design-author` agent now writes a dedicated `## Edge cases` section walking six categories (boundary values, invalid inputs, failure paths, concurrency, authorization, resource limits); the `structure-planner` pulls those scenarios into each slice's acceptance tests; the `red-author` writes them with the same care as happy-path tests and reports any gap as an upstream structure defect rather than inventing tests. Reinforced by methodology updates in `test-first-development`, `technical-design-doc`, and `engineering-standards`.
