...
// (slashes, dots, whitespace, backslashes), caps length at 100 chars to
// bound filesystem path growth, and keeps writes confined to
// ~/.team/<topic>/.
const TOPIC_PATTERN = /^[a-z0-9_][a-z0-9_-]{0,99}$/;
...
export function statePath(topic) {
  if (typeof topic !== "string" || !TOPIC_PATTERN.test(topic)) {
    throw new Error(
      `Invalid topic '${topic}'. Must match /^[a-z0-9_][a-z0-9_-]{0,99}$/ (max 100 chars).`,
    );
  }
