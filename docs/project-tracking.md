...
A Team run (`/team`, or the individual `/team phase <name>` phases) maps onto the board
like this:
...
- **Picking up work** → the card moves to **In progress** **automatically** as
  ...
  **Ready**, **Backlog**, or **Bugs**. Give `/team` or `/team phase team-fix` a ticket id
...
- **Opening the PR** → the PR phase links the PR to the issue. This covers
  `/team phase team-pr`, the `/team` PR gate, and `/team phase team-fix` Ship. The link is
