...
| **Implement** | Per-slice R-G-R trio (`red-author` → red gate → `green-author` → green gate → `refactor-author` with optional commit) for every slice → 5 parallel reviewers + typed retry loop. |
