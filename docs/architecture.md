...
must equal that swept set — the user-invocable skills only, never every
`SKILL.md` on disk — in both directions, so a new skill nobody classified
and a stale key for a deleted one each fail the build.
