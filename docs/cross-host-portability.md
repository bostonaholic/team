...
was resolved by the playbook refactor, plus a cross-cutting recency caveat:
...
2. **Codex lists every skill in the `$` picker, so `user-invocable: false` is a
   Claude-Code-only guarantee (resolved by the playbook refactor).** Before the
   refactor, Team's registered methodology and `principle-*` skills were
...
The generated archives measure 493,516 bytes per dependent command and
12,831,416 bytes across 26 commands. Each decodes to 1,112,809 bytes before
