...
`skills/`, `opencode/`, or host manifest *content* — `.claude-plugin/`,
...
> context — typically a false deny.
...
2. No-ops if the GitHub release `vX.Y.Z` already exists (idempotent, so it is
