...
- **Plan** — Tactical implementation plan derived from the approved structure. Read by the per-slice trio (`red-author`, `green-author`, `refactor-author`); the `implementer` reads it only for review-fix context. Not human-gated.
- **Worktree** — Orchestrator prepares an isolated git worktree.
- **Implement** — Per-slice R-G-R trio: `red-author` (per slice) writes failing tests → mechanical red gate → `green-author` (per slice) writes the minimum code that turns the slice green → mechanical green gate (3-attempt cap; prior slices' tests must still pass) → `refactor-author` (per slice; commit optional, no-op produces no commit). After every slice completes, 5 parallel reviewers (`code-reviewer`, `security-reviewer`, `technical-writer`, `ux-reviewer`, `verifier`) run an aggregate hard gate with a typed failure-class retry loop (max 5 rounds). `implementer` is reserved for the review-fix loop on aggregate-gate failure.
