/**
 * Shared Team-pipeline state detection for the runtime hooks.
 *
 * session-start-recover.mjs (SessionStart) and pre-compact-anchor.mjs
 * (PreCompact) both scan the home docs/plans/<id>/ plus every git worktree's
 * docs/plans/<id>/ for the most recent active topic, infer the current phase
 * from artifact presence + git signals, and inject a recovery/anchor notice.
 * That logic was byte-identical in both hooks; it lives here once. Each hook
 * now only formats its own notice text.
 *
 * ID_RE + PHASE_FILES are kept in sync with the bash resolver
 * skills/qrspi-workflow/discover-topic.sh by the drift tripwire in
 * tests/discover-topic.test.ts; these functions are unit-tested in
 * tests/pipeline-state.test.ts.
 *
 * Every git call is wrapped so a missing git binary / non-repo cwd degrades to
 * a home-only scan; nothing exported here throws.
 */
...
export const PHASE_FILES = ["task", "questions", "research", "design", "structure", "plan"];
...
export async function inferPhase(dir, rootDir, id, hasWorktree) {
