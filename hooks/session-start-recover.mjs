 ...
 * recently modified snapshot or null. Inlined (not imported) to keep
 * slice 3's revert surface minimal — see the parallel helper in
 * pre-compact-anchor.mjs; both collapse in slice 7.
 */
...
function formatSnapshotRecovery(snapshot) {
...
async function main() {
  ...
  if (active && active.snapshot && active.snapshot.phase && active.snapshot.phase !== "SHIPPED") {
    const additionalContext = formatSnapshotRecovery(active.snapshot);
