...
description: |
  ...
  "/groom-backlog".
effort: high
argument-hint: "scan [<project-number-or-url>] | promote <issue-number> [<project-number-or-url>]"
...
`$ARGUMENTS` must start with exactly one mode:

- `scan [<project-number-or-url>]` runs the board pass;
- `promote <issue-number> [<project-number-or-url>]` runs one-card promotion.

Missing modes or targets, extra tokens, malformed URLs, and non-numeric
numbers stop before reads. No project reference means run
...
Send the raw arguments on stdin. Use its JSON result; non-zero stops.

`promote` skips the board pass. Its optional project reference only constrains
membership. `scan` runs board mode.
...
Each write gets at most three attempts. After a failure, first re-read the
target and stop or skip if the result is ambiguous or the intended state now
exists. Classify only rate limits, timeouts, HTTP 429, and HTTP 5xx as
retryable. Pass `{attempt, retryable}` on stdin to
`node "<skill-dir>/scripts/retry.mjs"`; obey its action and 2- then 4-second
delay. Attempt three and every non-retryable failure stop the run. Report the
verified successful prefix and resumable plan path.
...
the `promote <issue-number>` mode is selected. It performs a narrow one-card load,
