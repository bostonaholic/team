...
not on the board stops non-zero and does not guess.
...
it found and asks which to groom. Promotion mode is not a board-mode run: it names one issue,
creates no grouping construct, and takes its repository from the issue itself.
...
instead. **`--promote` absent → board mode**, which runs steps 1–11 below.
