...
description: Adversarially review a technical design document with fresh context. Dispatches the built-in read-only `Explore` subagent (clean context, no shared history with the design-author) against `docs/plans/<id>/design.md` and presents its verdict — APPROVE, REQUEST CHANGES, or COMMENT. An independent standalone audit; the pipeline runs the same `design-review-brief` directly. Trigger on "review the design doc", "audit design.md", "is this design ready", or `/eng-design-doc-review`.
effort: high
...
Adversarially review a design document with fresh context. This skill is
an independent, fresh-context audit of a design document, run whenever you
want one. The pipeline's DESIGN review gate runs the same review through
`design-review-brief` without passing through this skill.
...
from `design-review-brief` and dispatches the built-in read-only
...
2. **Run the external cross-model pass.** Call the Skill tool with
   ...
   holding the fenced blocks to `design-review-brief`. Any skip
...
3. **Dispatch the review.** Call the Skill tool with
   `design-review-brief`, then call the `Agent` tool with
   `subagent_type: Explore`, the built-in read-only agent type. Pass that
   brief as the prompt, with every `<artifact-dir>` placeholder in it
   replaced by the artifact directory `$ARGUMENTS` resolved to. Do **not**
   define or reference a project agent —
...
- `design-review-brief` is **the pipeline's DESIGN review gate**
  (`skills/team/SKILL.md` and `/team-design` load it directly, as this
  skill does). Editing it changes pipeline behavior — treat any change to
  its headings, process, or verdict set as a pipeline change.
- This skill is **read-only, structurally for writes**. The `Explore`
  ...
  governed by `design-review-brief`'s read-only instruction, and that
  residual is
  ...
  *orchestrator* records the verdict to `design-review-<n>.md` when the
  pipeline gate runs. The recovery hooks fail closed on anything
