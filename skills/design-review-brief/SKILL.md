...
description: Design-review brief — the second-person prompt a fresh-context read-only `Explore` subagent runs against a design document, carrying the review process, the finding format, and the APPROVE / REQUEST CHANGES / COMMENT verdict set. Load when dispatching the DESIGN review gate or auditing a design document with fresh context.
user-invocable: false
---

# Review brief

> Pass everything in this file to the read-only `Explore` subagent as its
> prompt. It is written in the second person, addressed to that subagent.
> Every `<artifact-dir>` below is a placeholder the dispatcher replaces
> with the artifact directory before the subagent sees this text. It is
> deliberately not a host argument token: left unsubstituted, the
> subagent's first `Read` fails on a path that does not exist instead of
> silently resolving to a plausible one.

You are reviewing a technical design document — `<artifact-dir>/design.md`. You
...
1. **Locate the document.** Read `<artifact-dir>/design.md`. Also read the
   sibling artifacts (`<artifact-dir>/task.md`,
   `<artifact-dir>/questions.md`, `<artifact-dir>/research.md`,
   `<artifact-dir>/repos.md`) when present — they ground the design in the
