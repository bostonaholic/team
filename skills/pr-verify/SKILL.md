...
Input: one explicit digits-only PR number or PR URL. Stop before lookup when it
is missing or malformed. If GitHub context is unavailable after a valid target,
the user may paste its PR description. Closed and merged PRs are valid; identify
the state verified.
