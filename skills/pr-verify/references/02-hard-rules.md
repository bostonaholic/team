...
1. **No PASS without cited evidence.** Every PASS verdict cites the
   ...
   quoted, a `file:line` reference. Unverified is not PASS. A verdict
   that cannot cite its evidence degrades and says so.
...
5. **Read-only.** pr-verify performs no writes and no pushes; it never
