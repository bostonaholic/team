...
description: 'Triages unresolved PR review comments. Trigger on "address PR comments", "triage PR feedback", or "/pr-open-comments"; never infer triage intent from unresolved comments.'
effort: high
...
Call the Skill tool with `decision-making` for a thread with two or more
viable responses, naming the user as the decision owner. Use its result as
the recommendation. When the choice classifies as a one-way door,
`decision-making` returns the framed choice and its options instead of
