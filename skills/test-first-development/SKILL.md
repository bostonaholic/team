...
The fence's cross-gate canon: `skills/principle-scope-fence/SKILL.md` — the artifact authorizes exactly the change it names.
...
distinction matters.
The failure check is a mechanical gate — deterministic, running whether or not the model cooperates (`skills/principle-mechanical-gates/SKILL.md`).
...
Every acceptance test written under this skill follows the style rules in
`skills/test-style/SKILL.md` — behavior-not-implementation, DAMP setup,
narrow assertions, actionable failures, the deterministic-input rules
(control the clock, seed all randomness, own your state, impose order,
hermetic boundaries), the fidelity ladder, and the audit checklist. Before
confirming failures, audit each test against that skill's checklist and
cite the failing check by name.
