...
4. **A trailing run of standalone absolute-URL image lines.** That is the
   ...
   in an image line, and losing it is worse than a duplicate. The splice leaves
   this trailing tail in place by construction.

**Step B — attach one file per command.**

Validate each entry's `path` first. The set is exhaustive: the file **exists**,
is a **regular file**, its path holds no **newline**, and its path holds no
`#`. The last one is not cosmetic — the host reads `#` in an attach argument as
the alt-text delimiter, so a path such as one ending `login.png#after.png`
would upload a different file under an alt the caller never chose. An entry
failing any check is a failure and the loop continues.
...
entry's resolved absolute URL. Take whatever absolute URL appears there and
bind it to that entry. An empty suffix means the entry did not land.
...
```bash
node "<skill-dir>/splice.mjs" --body-file "$PRE_IMAGE_FILE" --section-file "$SECTION_FILE" > "$NEW_BODY_FILE"
```

Exit 0 writes the new body on stdout. Exit 1 prints `unchanged: <reason>` on
stderr and means no rule allowed the write: report the reason and leave the
body alone. On exit 0, one write lands it, and that same write also clears the
tails the attach step appended:
...
A run where every entry failed carries no resolved form at all, only the
degraded one.

The degraded form renders each local path as **plain text**:

```markdown
## Screenshots

**<caption>** (<state>) — captured, not yet uploaded: <local path>
