...
PARENT="${P%/*}"
# Delete from inside the verified directory: a symlink swapped in above it after
# the check cannot redirect rm, and rm does not follow the final component.
cd -P -- "$PARENT" || refuse "cannot be checked: its directory does not resolve"
if [ "$(pwd -P)" != "$REAL_ROOT${PARENT#"$TMPROOT"}" ]; then
