...
description: |
  ...
  red, or unreviewed — and run Mode B ONLY on an explicit user abandon
  request. A dispatch from a running /shipit satisfies the cleanup gate for
  Mode A only. Mode A cues: the user says "the PR was merged", "clean up the
  branch", "delete the merged branch", or runs "/pr-cleanup"; Mode A verifies
  the PR actually merged, removes the branch's worktree, resyncs the default
  branch, and deletes the local branch. Mode B cues: the user says "close the
  PR", "close those PRs", "abandon this", or "scrap it"; Mode B closes the
  PR(s), then deletes every trace — worktree, local and remote branches,
  planning scratch.
effort: medium
