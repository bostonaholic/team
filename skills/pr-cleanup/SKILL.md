...
- **Mode A — merged.** The work landed upstream: remove the branch's
  ...
  A squash merge leaves the branch unmerged in git's eyes, so
  `git branch -d` refuses; the merged-PR gate below is what makes `-D`
  acceptable.
...
4. [Step 0 — resolve and validate $PRIMARY_ROOT](references/05-step-0-resolve-and-validate-primary-root.md)
5. [Step 1 — detect the default branch](references/06-step-1-detect-the-default-branch.md)
6. [Step 2 — resolve targets, refuse protected names](references/07-step-2-resolve-targets-refuse-protected-names.md)
