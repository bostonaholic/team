...
7. **"Delete the worktree" means the git worktree, never the primary
   clone.** Step 0's validated `$PRIMARY_ROOT` backs this by detection,
   not path convention.
...
9. **Every command is anchored.** After step 0, every git command runs as
   ...
   destructive commands take `$PRIMARY_ROOT`-absolute paths. Step 0's end
   lists every other anchor, and that list is closed.
...
11. **No destructive command, and no gate protecting one, relies on a
    variable set in an earlier Bash invocation.** Every invocation that
    uses `$PRIMARY_ROOT`, `$REPO`, or `$DEFAULT` re-derives them (the
    step 0 block for the first two, step 1 for `$DEFAULT`) in that same
    invocation, and every expansion a destructive command or a gate
    depends on uses the `${VAR:?}` form so an unset variable aborts
    instead of expanding to empty. An empty `$DEFAULT` fails silently: it
    drops the default branch out of step 2's protected-name pattern,
    leaving `main` deletable. Inside `$( )` a `${VAR:?}` kills only the
    subshell and the parent continues with an empty value, so guard a
