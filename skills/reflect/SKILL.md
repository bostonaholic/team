...
`$ARGUMENTS` is empty or one lowercase hyphenated skill name. Pass it exactly
as stdin data to the validator before reading the session; never put it in
shell text or argv:
...
Non-zero stops. For a returned focus, inspect only `skills/<focus>` and
`.claude/skills/<focus>`. If neither exists, print near matches and stop; search
only those two skill roots. Carry a valid focus into every lens as scope, not
as a presumed fault.
