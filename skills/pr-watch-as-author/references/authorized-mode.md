...
For each newly detected batch, use the inline triage procedure named in
`pr-watch-as-author`; do not invoke the explicit-only `pr-open-comments`
command. Verification and safety stops remain mandatory even though confidence
no longer gates execution.
