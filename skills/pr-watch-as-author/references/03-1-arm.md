...
- If `gh pr ready` fails, warn and keep watching — the promotion is not a
...
- Take a baseline snapshot from the shared pull-request comment retrieval:
  ...
  `reviewDecision`. Record comment ids, not just the latest timestamp. The
  set of triaged feedback ids is what makes triage idempotent across cycles.
...
- Comments authored by you are never feedback to yourself — exclude the
  ...
  poll. Everyone else's count, bots included.
