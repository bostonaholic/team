...
3. **Mechanical gate** — confirm all tests fail with assertion errors
   (not crashes), the test-architect reports `Expected results derived
   independently: YES`, **and** every static check the project defines passes
   ...
   back to the `test-architect`. If the report says `Expected results derived
   independently: NO`, return it to the `test-architect` too. A runner that
   executes tests without type-checking them leaves a red type checker behind
   a green suite, and the next actor to notice is the `verifier`, a full review
   round later.
