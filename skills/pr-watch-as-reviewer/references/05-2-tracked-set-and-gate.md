...
through the step-4 poll query. Its selection set carries every field this
partition reads. Partition them client-side into two classes:
...
- Threads from the viewer's PENDING (unsubmitted) review stay excluded
  ...
  so counting them would deadlock the watch until the soft cap.
  Plain comments have no unsubmitted state — posting one publishes it. A
  review summary joins only when its review is submitted. (GitHub's PENDING
  review state means "not yet submitted"; the step-4 **pending** re-review
  verdict means "not yet settled".)
...
- A thread resolved by the skill and a thread resolved by the author are
  worth exactly the same at approval time — both need a passing verdict,
  and neither is credited for the resolve itself. A thread the skill
  resolved on a verdict that a later push voids (step 6's re-check) is
  back to needing a fresh verdict even though its resolved bit never
