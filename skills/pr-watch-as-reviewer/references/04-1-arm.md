...
- On the loop path with auto-merge enabled at arm, warn loudly that the
  ...
  confirmation before you arm. Ask the user to confirm the unattended
  run. Treat a "no" as a refusal to arm, never
  a silent downgrade to a watch that skips the approval. The warning
