...
gate, and the current auto-merge state. Then continue polling. The
arm-time baselines are the values GitHub cannot return — recover them
from the transcript:

- the **arm-time head SHA** (arm report, every snapshot line). When no
  copy survives, step 6's fail-closed rule applies.
- the **arm-time auto-merge state and if its confirmation was granted**
  ...
  confirmation.
- the **arm-time tracked count**, per shape (arm report, cycle-0
  snapshot). When unrecoverable, say so in the approval body in place of
  the count comparison.
- the **tracked PR-level item list** — review summaries and conversation
  comments from the step-1 classification (arm report, by url). This one
  is *not* re-derivable: re-running
  ...
  comment list was lost and offer to re-arm. A watch that cannot say
  what it is tracking must not approve.
- **which replies were already rebutted** — re-derivable from GitHub:
  the viewer's own replies are on the thread, so the last one shows which
  of the author's replies has already been answered. Nothing is written
  for a reply that already carries a rebuttal beneath it. Prefer GitHub
  over the transcript when the two disagree.
- **which threads the skill resolved** versus the author (snapshot
  lines), for the `<R>` disclosure in the approval body. When
  unrecoverable, say so in the body in place of the count rather than
  attributing the resolves either way.
...
- the stop reason, by its step-5 name
...
- when an approval was cast: its URL, the cited head SHA, and the
  ...
  url, its shape, whether it was addressed or answered, the reaction
  that verdict placed, and who resolved it — you or the author); both
  SHAs and a drift note when the head moved between arm and approval;
  both counts for any shape whose tracked count changed between arm and
  approval
- the write ledger, on every path: how many threads the skill resolved,
  ...
  reactions it placed — reported whether or not an approval was cast
