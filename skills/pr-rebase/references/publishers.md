...
Before delegation, run `scripts/context.mjs remote-head` against the binding's
validated push URL and compare it with the recorded pre-fetch SHA. Report the
residual race because the wrapper cannot accept git's exact lease. Capture and
compare draft state around the publisher. Never fall back to `git push` when
