...
// file becomes an identity-only wrapper (64-90 lines) whose procedure lives
...
const EXPECTED_PRINCIPLE_COUNT = 9;
...
describe("principle-* family contract (tests 2-7) at expected member count 9", () => {
  ...
  test("moved exactly once: each moved run is gone from the source skill it left", () => {
    ...
    for (const { owner, source, run } of ACTIVE_MOVED_RUNS) {
      ...
      if (normalizeForMarkers(content).includes(marker)) {
        offenders.push(`${source}: still holds the run that moved to ${owner} — "${marker.slice(0, 64)}…"`);
      }
    }
    expect(offenders).toEqual([]);
  });
  ...
  test("moved exactly once: no moved run has a second home in another skill", () => {
    ...
    for (const { owner, source, run } of ACTIVE_MOVED_RUNS) {
      ...
      for (const skill of everySkill) {
        ...
        if (normalizeForMarkers(content).includes(marker)) {
          offenders.push(`${skill}: holds a second copy of the run owned by ${owner} — "${marker.slice(0, 64)}…"`);
        }
      }
    }
    expect(offenders).toEqual([]);
...
describe("thin agents: documentation counts agree at 65 skills", () => {
  const SKILLS_MD = join(REPO_ROOT, "docs", "skills.md");
  const ARCHITECTURE_MD = join(REPO_ROOT, "docs", "architecture.md");

  test("skills/ holds exactly 65 SKILL.md files", () => {
  ...
  test("AGENTS.md heading reads Skills (65)", () => {
    expect(read(join(REPO_ROOT, "AGENTS.md"))).toContain("## Skills (65)");
  });

  test("docs/skills.md description counts 65 skills and no stale 56", () => {
    const flattened = read(SKILLS_MD).replace(/\s+/g, " ");
    expect(flattened).toContain("65 skills");
  ...
  test("docs/skills.md split sentence sums to 65", () => {
    expect(read(SKILLS_MD).replace(/\s+/g, " ")).toContain(
      "11 pipeline entry-point + 8 standalone utility + 46 methodology = 65",
    );
  });

  test("docs/architecture.md counts all 65 skills in both locations and no stale 55/54/53/51/31", () => {
    const content = read(ARCHITECTURE_MD);
    expect(content).toContain("all 65 skills");
  ...
  test("README.md counts 65 entry-point + methodology skills", () => {
    expect(read(join(REPO_ROOT, "README.md")).replace(/\s+/g, " ")).toContain(
      "65 entry-point + methodology skills",
    );
  });

  test("README.md counts all 65 skills in the Antigravity install line", () => {
    expect(read(join(REPO_ROOT, "README.md")).replace(/\s+/g, " ")).toContain(
      "all 65 skills",
    );
  });

  test("docs/cross-host-portability.md counts 65 skills and no stale 56", () => {
  ...
  test("docs/index.md counts all 65 skills", () => {
    expect(read(join(REPO_ROOT, "docs", "index.md")).replace(/\s+/g, " ")).toContain(
      "all 65 skills",
    );
  });

  test("docs/skills.md counts 46 methodology skills and no stale 37", () => {
    const flattened = read(SKILLS_MD).replace(/\s+/g, " ");
    expect(flattened).toContain("The 46 methodology skills");
