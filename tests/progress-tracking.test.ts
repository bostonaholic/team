...
describe("skill count reconciliation (-> 75: nineteen principle-* skills added to the 56-skill library)", () => {
  const CLAUDE_MD = join(REPO_ROOT, "CLAUDE.md");
  const AGENTS_MD = join(REPO_ROOT, "AGENTS.md");

  test("CLAUDE.md heading reads '## Skills (75)'", () => {
    expect(/^## Skills \(75\)/m.test(read(CLAUDE_MD))).toBe(true);
  });

  test("AGENTS.md heading reads '## Skills (75)'", () => {
    expect(/^## Skills \(75\)/m.test(read(AGENTS_MD))).toBe(true);
  });

  test("filesystem has exactly 75 SKILL.md files declaring a name:", () => {
    ...
    // machine-local teardown skill) plus nineteen members of the
    // single-claim principle-* family, which take the count to 75.
    const dirs = readdirSync(SKILLS_DIR, { withFileTypes: true })
      .filter((d) => d.isDirectory())
      .map((d) => join(SKILLS_DIR, d.name, "SKILL.md"))
      .filter((p) => existsSync(p));
    const withName = dirs.filter((p) => /^name:/m.test(read(p)));
    expect(withName.length).toBe(75);
  });
});
