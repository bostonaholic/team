...
const PARSER = join(ROOT, "skills", "groom-backlog", "scripts", "parse-input.mjs");
const body = () => (existsSync(PATH) ? read(PATH) : "");
const flat = (value: string) => value.replace(/\s+/g, " ");

describe("groom-backlog public contract", () => {
  test("keeps invocation, project argument, and promotion mode", () => {
    const fm = frontmatter(body());
    expect(fm).toMatch(/^name:\s*groom-backlog$/m);
    expect(fm).toMatch(/^effort:\s*high$/m);
    expect(fm).toMatch(/^argument-hint:.*--promote <issue-number>/m);
  ...
  test("input parser rejects ambiguity and dispatches modes", () => {
    expect(parseInput("7")).toMatchObject({ ok: true, mode: "board", promote: null });
    expect(parseInput("--promote 42")).toMatchObject({ ok: true, mode: "promotion", promote: 42 });
    expect(parseInput("7 8").ok).toBe(false);
    expect(parseInput("--promote x").ok).toBe(false);
    expect(parseInput("0").ok).toBe(false);
    expect(parseInput("--promote 9007199254740992").ok).toBe(false);
  });

  test("reads raw arguments only from stdin without executing them", () => {
    const directory = mkdtempSync(join(tmpdir(), "team-groom-input-"));
    const marker = join(directory, "injected");
    try {
      const valid = spawnSync(process.execPath, [PARSER], { input: "--promote 42", encoding: "utf8" });
      expect(valid.status).toBe(0);
      expect(JSON.parse(valid.stdout)).toMatchObject({ mode: "promotion", promote: 42 });

      const injected = spawnSync(process.execPath, [PARSER], {
        input: `--promote 42; touch ${marker}`,
        encoding: "utf8",
      });
      expect(injected.status).toBe(2);
      expect(existsSync(marker)).toBe(false);
      expect(body()).not.toContain('parse-input.mjs" "$ARGUMENTS"');
    } finally {
  ...
  test("executes in dependency order with pre-images and re-reads", () => {
    ...
    expect(text).toMatch(/at most\s+twice \(three total attempts\).*2 then 4 seconds/is);
    expect(text).toMatch(/Never blindly retry an\s+issue creation, comment, or close/is);
  });
