...
describe("pr-verify skill contract", () => {
  ...
  test("requires an explicit PR while retaining paste fallback and merged verification", () => {
    const text = body();
    expect(text).toContain("explicit digits-only PR number or PR URL");
    expect(text).toContain("missing or malformed");
    expect(text).not.toContain("current branch's PR");
    expect(text).toContain("paste its PR description");
