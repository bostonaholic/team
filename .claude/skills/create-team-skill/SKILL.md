...
- `Used by` lists, in codepoint order, every skill whose `Uses` list names this skill. A guarded principle lists, in codepoint order, every skill whose files read its `SKILL.md` by path — the read is what applies it.
- `Uses` lists only the skills the files load through ``Call the Skill tool with `<name>` ``, in any `.md` under `skills/<name>/`, references and prompt templates included. A citation is not a use, so a skill it merely names by path or in prose stays off the list — the guarded-principle path read above is the one reference that counts. Sort in codepoint order (plain `sort`, so `pr-verify` precedes `reviewing-code`). `tests/docs-skills-catalog.test.ts` is the gate.
