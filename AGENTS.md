...
| `pre-merge-guard.mjs` | PreToolUse(Bash) | Deny `gh pr merge` when the version-bump invariant fails. Its reach is one surface: a literal `gh pr merge` Bash tool call in a session that loaded `.claude/settings.json`. UI, raw-terminal, and wrapped merges are outside that reach and carry their own gates (see [docs/versioning.md](docs/versioning.md)) |
