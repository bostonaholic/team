...
See `skills/*/SKILL.md`. Entry point skills double as slash commands. Some of them are standalone slash-command utilities that are not QRSPI phases. `shipit` lands a reviewed PR — on stated ship intent, never on a PR merely being approved or green. `pr-open-comments` triages unresolved PR review feedback — on stated triage intent, because an auto-applied item commits and pushes. `pr-watch-as-author` is a bounded PR review watch loop — on stated watch intent, because it undrafts the PR and moves the ticket. `pr-watch-as-reviewer` is the reviewer-side watch-and-approve — on stated watch intent, because an approval can transitively arm an auto-merge. `groom-backlog` grooms a project backlog with a board-level pass plus per-item promotion, and can close an issue whose premise evaporated — an irreversible public mutation, each close gated on its own per-issue approval. `pr-cleanup` tears down local and remote branch state after a PR is merged or abandoned — abandon mode on the user's own stated request, never on a PR merely being stale. `pr-verify` verifies a PR's test plan with evidence-rated verdicts. `pr-screenshots` attaches local images to any PR's body and owns every upload mechanic `team-pr` used to duplicate — on stated intent to add screenshots or attach an image to a PR, never inferred from images sitting on disk. `pr-rebase` rebases a branch onto its base, resolving conflicts from both sides' intent and comparing the project's checks before and after — user-invoked only (`disable-model-invocation: true`), on stated rebase intent, never on a branch merely being behind. `retro` mines the invoking session's own transcript for durable learnings and turns the approved ones into skill edits and tracker issues — user-invoked only (`disable-model-invocation: true`), because a run rewrites `SKILL.md` files and files public issues. `why` investigates the design rationale behind code — parallel read-only investigators over every available evidence source, synthesized into confidence-tiered, citation-backed findings. `how` explains how a subsystem works at onboarding depth, with an optional fresh-context architectural critique; the two are companions (mechanics vs. motivation), both read-only. `no-comments` removes low-value source comments after a fresh read-only review and asks before encoding constraints — user-invoked only (`disable-model-invocation: true`), on stated comment-cleanup intent, never inferred from a diff containing comments. `agent-prompt` composes a self-contained, agent-optimized prompt for a task from a short description and an optional target repo, emitted as text with no dispatch or execution. Methodology skills are loaded by agents. Shared principles live in six ordinary documents under `skills/team/principles/`. Guarded principles are commands that set `disable-model-invocation: true` and are read by path (`principle-fix-root-causes`), never applied on their own. Execution, decisions, external-data, and bug-fix rules have scoped resource owners. Question, Research, Design, Structure, Plan, Implement, and bug-fix procedures live in playbooks with shared templates and dependency, testing, and diagnosis references. Consumers read applicable installed paths before work. Twelve agents read execution rules from their bodies; file-finder keeps its single-step contract. The 28 commands remain registered. For design guidelines on skill extraction and load limits, see [`docs/architecture.md`](docs/architecture.md#design-guidelines).
...
- **Todo-first progress tracking.** Any agent or skill that executes a multi-step numbered procedure seeds one TodoWrite item per step before starting and marks each complete as it goes. See [execution rules](skills/team/references/execution.md) for the convention and ledger-ownership rules.
- **A drafted PR carries no version. Nothing versions until the step immediately before the merge command.** Bullets accumulate under `## [Unreleased]`; the version strings, the dated changelog section, and the `vX.Y.Z` PR-title prefix all stay untouched until that step. **"Land time" means exactly that moment, never "when the work is done"** — a version assigned at PR-open time is computed against a `main` that keeps moving, so it goes stale as soon as another PR lands and the pre-merge guard then denies the merge. Versioning runs through the `version-bump` skill, which fires **only on explicit land intent** and never on work merely looking landable. **The bump is also conditional on a runtime change, not universal:** only a PR that changes the **distributed runtime surface** (per the runtime-vs-development split above) bumps; a dev-only PR (CI, docs, `.claude/` tooling) lands with no bump, no changelog cut, and a plain conventional title. The deterministic gate `.github/scripts/version-bump-required.sh` states a *merge* precondition, so its exit 1 on an unbumped runtime branch is the expected state throughout review rather than a cue to bump; `version-bump` runs it early, and the pre-merge dev hook (`.claude/hooks/pre-merge-guard.mjs`) denies the merge command on either violation. Full land-time procedure: `skills/shipit/SKILL.md` invokes `skills/version-bump/SKILL.md`, then pushes, waits for CI, and merges. See [docs/versioning.md](docs/versioning.md).
